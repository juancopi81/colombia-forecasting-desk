# Shadow Forecast Experiment

**Status:** `collecting`
**Decision-grade runs:** 8 / 10
**Start date:** `2026-08-12`

> Descriptive only. This small sample does not establish model quality.

## Run Collection

- Remaining decision-grade runs: 2
- Latest counted run: 2026-08-21
- Missing agent analysis: 0
- Invalid agent analysis: 0
- Non-decision-grade runs skipped: 0

## Dispositions

| Disposition | Runs |
| --- | ---: |
| `insight_only` | 6 |
| `shadow_track` | 2 |

## Review Compliance

- Bounded follow-ups completed: 8 / 8 (100.00%)
- Triggered tension cards reviewed: 14 / 14 (100.00%)

## Shadow Forecasts

- Created: 2
- Open: 0
- Resolved: 2
- Overdue: 0
- Complete baseline fields: 2 / 2
- Resolved forecasts with Brier score: 2
- Resolved forecasts with comparable model/baseline scores: 2
- Mean model Brier score (all scored resolved rows): 0.1604
- Mean model Brier score (paired rows only): 0.1604
- Mean baseline Brier score (paired rows only): 0.2339
- Mean Brier improvement vs baseline (positive is better): +0.0735
